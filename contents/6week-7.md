...
4개의 스택을 이용해서 순열 A의 모든 원소들을 삽입 한 뒤에

4개의 스택에중 원하는 스택에서 수를 꺼내어서 
...
따라서 이 문제의 핵심은 순열 A에서 Stack 4개로 이동 시키는 과정에서 스택 4개가 모두 정렬된 형태로 
...
내부 for 문은 스택 개수 만큼 반복하여 모든 스택을 순회하면서,

A의 원소가 스택에 담긴 데이터 보다 클 경우에는 그 Stack에 삽입하고,

그렇지 않으면 다음 스택에 방문해서 크기 비교를 한다.

모두 비교해서 스택에 정렬된 형태로 담기지 못하는 데이터가 있을경우에는 `check = false` 바꿔주고 NO를 출력한다.
