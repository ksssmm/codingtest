...
시간 순서대로 손님을 받기 위해서 시작시간을 기준으로 정렬된 예약 시간배열이 필요해서 정렬했다.
...
`rooms`에 진행중인 예약을 추가 하면서, 현재 예약의 종료시간이 되면 다음 예약으로 대체하는 방식으로 풀었다.

종료시간이 빠른 데이터 부터 대체 해야 하므로 `PriorityQueue`를 사용했다.
